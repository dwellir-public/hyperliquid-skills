...
- **Connect to Dwellir's Hyperliquid endpoints** — correct hostnames, auth methods, and connection patterns
- **Query the Info API** — which types are proxied vs public-only, with working code examples
- **Stream order book data** — L2/L4 subscriptions via Dwellir's WebSocket with up to 100 levels of depth
- **Use L1 gRPC streaming** — block, fill, and order book snapshot streams from HyperCore
- **Deploy on HyperEVM** — ethers.js and viem setup with chain ID 999
- **Place orders via native API** — Hyperliquid's Exchange API with the Python SDK
- **Choose the right endpoint** — read (Dwellir) vs write (Hyperliquid native) architecture
...
Sign up at [dashboard.dwellir.com](https://dashboard.dwellir.com) to get an API key. The HyperEVM JSON-RPC and Info API proxy are included in the base plan. The gRPC gateway and order book server are premium add-ons.
